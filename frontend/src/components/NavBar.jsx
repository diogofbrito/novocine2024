...
export function NavBar() {
    ...
    const isFilmDetailPage =
        [langRouteMap.EN.arquivo, langRouteMap.PT.archive].includes(
    ...
    return (
        <ThemeProvider>
            <>
                <div
                    className={`fixed z-50 top-0 px-[4.5rem] iphone:px-[1rem] flex w-screen h-[4.5rem] items-center opacity-0 mobileNavBar ${
                        (animationDone || isNotHomePage) && "!opacity-100"
                    }`}
                    ref={navbarRef}
                >
                    {!isFilmDetailPage ? (
                        <div className="grid-cols-3 w-full items-center grid ">
                            ...
                            <div className="justify-center flex  iphone:mt-0 ">
                                <NavLink
									to={`/${routeTranslations.home[lang]}`}
                                    className="font-cine text-5xl -mt-1"
                                >
                                    NOVOCiNE
                                </NavLink>
                            </div>
